class_name StorylineIntro
extends Control

signal storyline_completed

const FRAME_PATHS: Array[String] = [
	"res://assets/storyline/start/storyline_1.png",
	"res://assets/storyline/start/storyline_2.png",
	"res://assets/storyline/start/storyline_3.png",
	"res://assets/storyline/start/storyline_4.png",
	"res://assets/storyline/start/storyline_5.png",
	"res://assets/storyline/start/storyline_6.png",
	"res://assets/storyline/start/storyline_7.png",
	"res://assets/storyline/start/storyline_8.png",
	"res://assets/storyline/start/storyline_9.png"
]

const NEXT_SCENE: String = "res://scenes/levels/outdoor/outdoor_map.tscn"
...
func _ready() -> void:
	# 1. Preload textures safely with fallback
	_load_all_textures()
	
	# 2. Connect HUD button signals idempotently
	...
	# 4. Smooth startup fade-in from black
	if fade_overlay:
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("ui_cancel") or (event is InputEventKey and event.pressed and not event.is_echo() and event.keycode == KEY_ESCAPE):
		...
		_on_prev_pressed()
	elif event.is_action_pressed("ui_accept") or event.is_action_pressed("interact") or event.is_action_pressed("ui_right") or (event is InputEventKey and event.pressed and not event.is_echo() and (event.keycode == KEY_SPACE or event.keycode == KEY_ENTER or event.keycode == KEY_D or event.keycode == KEY_RIGHT or event.keycode == KEY_E)):
		_on_next_pressed()
...
func _load_all_textures() -> void:
	...
	for path in FRAME_PATHS:
		...
		if ResourceLoader.exists(path):
			tex = load(path)
		elif path.ends_with("storyline_7.png") and ResourceLoader.exists("res://assets/storyline/start/soryline_7.png"):
			tex = load("res://assets/storyline/start/soryline_7.png")
			
		if tex:
			textures.append(tex)
		else:
			push_warning("Storyline frame texture missing: %s" % path)
...
func _update_hud() -> void:
	...
	if next_btn:
		if current_index == textures.size() - 1:
			next_btn.text = "Begin Journey [Space] »"
		else:
