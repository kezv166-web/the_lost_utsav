...
def build_tres(output_path, anim_defs):
    all_textures = []
...
"""
    ...
    print(f"Successfully created {output_path} with {len(anim_defs)} animations and {len(all_textures)} textures.")

# 1. Asur Character Animations
asur_anims = [
    # Idles (2 frames each)
    ...
    # Gadha Attacks (cols 0..3: windup, overhead raise, swing down, floor slam hold)
    ("attack_down", [f"asur_attack_down_{i}.png" for i in range(4)] + ["asur_attack_down_3.png"], 6.0, "false"),
    ("attack_left", [f"asur_attack_left_{i}.png" for i in range(4)] + ["asur_attack_left_3.png"], 6.0, "false"),
    ("attack_right", [f"asur_attack_right_{i}.png" for i in range(4)] + ["asur_attack_right_3.png"], 6.0, "false"),

    # Mega Quake Slam / Stomp Attacks (8 frames each: ready, raise, windup, swing, impact slam, quake hold, settle, recover)
    ("stomp_down", [f"asur_stomp_down_{i}.png" for i in range(8)], 7.5, "false"),
    ("stomp_left", [f"asur_stomp_left_{i}.png" for i in range(8)], 7.5, "false"),
    ("stomp_right", [f"asur_stomp_right_{i}.png" for i in range(8)], 7.5, "false"),
]

build_tres("scenes/enemy/asur_sprite_frames.tres", asur_anims)

# 2. Ground Spikes Shockwave Effect Animations (cols 4..7)
spike_anims = [
    ("spikes_down", [f"asur_spikes_down_{i}.png" for i in range(4)], 7.0, "false"),
    ("spikes_left", [f"asur_spikes_left_{i}.png" for i in range(4)], 7.0, "false"),
    ("spikes_right", [f"asur_spikes_right_{i}.png" for i in range(4)], 7.0, "false"),
]

build_tres("scenes/enemy/asur_spikes_sprite_frames.tres", spike_anims)
